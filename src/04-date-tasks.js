...
function timeSpanToString(startDate, endDate) {
  ...
  let hrs = Math.floor(d / msIn.hr);
  d -= hrs * msIn.hr;

  let min = Math.floor(d / msIn.min);
  d -= min * msIn.min;

  let sec = Math.floor(d / msIn.sec);
  d -= sec * msIn.sec;

  let ms = Math.floor(d / msIn.ms);
...
function angleBetweenClockHands(date) {
  ...
  let diffGrad = Math.abs(hrsGrad - minsGrad);
